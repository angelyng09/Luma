...
struct CreateAccountPageView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    textInputRow(
                        label: L10n.tr("auth.entry.field.email.label"),
                        placeholder: L10n.tr("auth.entry.field.email.placeholder"),
                        text: $email,
                        field: .email,
                        isSecure: false
                    )
                    ...
                    textInputRow(
                        label: L10n.tr("auth.entry.field.username.label"),
                        placeholder: L10n.tr("auth.entry.field.username.placeholder"),
                        text: $username,
                        field: .username,
                        isSecure: false
                    )
                    ...
                    Button {
                        ...
                        Text(isSubmitting ? L10n.tr("auth.entry.button.submitting") : L10n.tr("auth.entry.button.sign_up"))
                    }
        ...
        .lumaScreenStyle()
        .navigationTitle(L10n.tr("sign.up.nav.title"))
        .inlineNavigationTitle()
        .onAppear {
            isTitleFocused = true
            AccessibilityAnnouncer.announce(L10n.tr("announcement.sign_up_page"))
        }
    }
    ...
    private func submit() {
        ...
        if trimmedEmail.isEmpty {
            let message = L10n.tr("auth.error.invalid_email")
            errorMessage = message
            AccessibilityAnnouncer.announce(message)
            return
        }

        if trimmedEmail.contains("@") == false || trimmedEmail.contains(".") == false {
            let message = L10n.tr("auth.error.invalid_email")
            errorMessage = message
            AccessibilityAnnouncer.announce(message)
            return
        ...
        Task { @MainActor in
            ...
            do {
                ...
                AccessibilityAnnouncer.announce(L10n.tr("announcement.sign_up_success"))
            } catch {
